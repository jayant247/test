...
//            $pageNo = $request->pageNo;
//            $skip = $limit*$pageNo;
            $orders = Order::with(['orderStatus','paymentStatus','orderItems','orderItems.productVariable','orderItems.productVariable.productDetails','addressDetails'])->whereIn('payment_status',[2,4])->whereUserId(Auth::user()->id)->orderBy('id','DESC')->get()->toArray();
            if(count($orders)>0){
                return $this->sendResponse($orders,'Data Fetched Successfully', true);
            }else{
                return $this->sendResponse([],'No Orders Available available', false);
